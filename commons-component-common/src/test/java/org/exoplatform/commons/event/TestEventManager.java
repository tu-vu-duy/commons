...
public class TestEventManager<S, D> extends BaseCommonsTestCase {
  private static final Log LOG = ExoLogger.getLogger(TestEventManager.class);
   ...
   * 2.
   * Test for remove all the registered listeners.
   *  Input:
  ...
  public void testManageEventListener() {
    ...
    assertEquals(2, dmsEventManager_.getEventListeners("TestDMSFileListener").size());
    ...
    assertEquals(0, dmsEventManager_.getEventListeners("TestDMSFileListener").size());
  }

  /**
   * Broadcast for DMSFile Input: - A listener registered to listen for DMSFile
   * object Output: - Broadcast event executed when file created - Broadcast
   * event executed when file updated - Broadcast event executed when file
   * deleted
   * 
  ...
  public void testBroadcastDMSEvent() throws PathNotFoundException, RepositoryException {
    ...
    assertTrue(dmsFileListener.fileCreated);
    ...
    assertTrue(dmsFileListener.fileUpdated);

    // Broadcast when a file removed
    testNode.remove();
    broadcastEvent(dmsFile, EventType.REMOVE);
    assertTrue(dmsFileListener.fileRemoved);
    session.save();
  ...
  private class DMSFile extends AbstractFile<DMSFile> implements File {
    ...
    @Override
    public DMSFile getParent() throws ItemNotFoundException, AccessDeniedException, PathNotFoundException, RepositoryException {
      return null;
    }
